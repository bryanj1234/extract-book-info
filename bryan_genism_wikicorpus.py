...
re_section_heading = re.compile(r'[^=](={2,6})([^=\n]+)\1[^=]')
"""BRYAN"""
...
def extract_pages(f, filter_namespaces=False, filter_articles=None, max_characters_in_doc_chunk=30000, chunk_by_page_section=False):
    ...
    for elem in elems:
        if elem.tag == page_tag:
            ...
            text = text or ""
            text_groups = [('ENTIRE_ARTICLE', text)]

            if chunk_by_page_section:
                heading_offsets = []
                search_res = re_section_heading.finditer(text)
                if search_res is not None:
                    for rec in search_res:
                        heading_level = len(rec[1])
                        heading_text = rec[2].strip()
                        heading_offset = rec.start()
                        heading_offsets.append((heading_offset, heading_level, heading_text))
                # Add offset 0 for the section at the top (before any header).
                heading_offsets.append((0, 0, 'BEGINNING'))
                # Sort by increasing offset
                heading_offsets = sorted(heading_offsets, key=lambda rec:rec[0])
                num_sections = len(heading_offsets)
                if num_sections > 0:
                    text_groups = []
                    for section_ind in range(num_sections):
                        rec = heading_offsets[section_ind]
                        heading_offset = rec[0]
                        heading_level = rec[1]
                        heading_text = rec[2]
                        section_text = False
                        heading_info = False
                        if section_ind < num_sections - 1:
                            end_offset = heading_offsets[section_ind + 1][0]
                            heading_info = str(heading_offset) + '-' + str(end_offset) + ':' + str(heading_level) + ':' + heading_text
                            section_text = text[heading_offset:end_offset]
                        else:
                            heading_info = str(heading_offset) + ':' + str(heading_level) + ':' + heading_text
                            section_text = text[heading_offset:]
                        text_groups.append((heading_info, section_text))

            for group_rec in text_groups:
                group_name = group_rec[0]
                group_text = group_rec[1]
                chunk_start_ind = 0
                while chunk_start_ind < len(group_text):
                    content_chunk = group_text[chunk_start_ind:chunk_start_ind + max_characters_in_doc_chunk]
                    tag = group_name + ':' + str(chunk_start_ind)
                    yield title, group_text or "", pageid, tag
...
def process_article(args, tokenizer_func=tokenize, token_min_len=TOKEN_MIN_LEN,
    ...
    text, lemmatize, title, pageid, chunk_start_ind = args
    ...
    return result, title, pageid, chunk_start_ind
...
class BryanWikiCorpus(TextCorpus):
    ...
    * <LANG>wiki-latest-pages-articles.xml.bz2
    ...
    def get_texts(self, max_characters_in_doc_chunk=30000, chunk_by_page_section=False):
        ...
        texts = \
            ((text, self.lemmatize, title, pageid, tag, tokenization_params)
             for title, text, pageid, tag
             in extract_pages(bz2.BZ2File(self.fname),
                                            self.filter_namespaces,
                                            self.filter_articles,
                                            max_characters_in_doc_chunk=max_characters_in_doc_chunk,
                                            chunk_by_page_section=chunk_by_page_section))
        ...
        try:
            ...
            for group in utils.chunkize(texts, chunksize=10 * self.processes, maxsize=1):
                for tokens, title, pageid, tag in pool.imap(_process_article, group):
                    articles_all += 1
